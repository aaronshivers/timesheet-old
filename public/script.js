...
class Job {
  constructor(customer, description, date, timeIn, timeOut, id = uuidv4()) {
...
class UI {
  ...
  static addJobToList(job) {
    ...
    const formattedTimeIn = moment(`2019-01-01T${ job.timeIn }`).format('h:mm a')
    const formattedTimeOut = moment(`2019-01-01T${ job.timeOut }`).format('h:mm a')
...
document.getElementById('job-form').addEventListener('submit', (event) => {
  ...
  if (!customer || !description) {
    ...
    // Instantiate job
    const job = new Job(customer, description, date, timeIn, timeOut, id)
